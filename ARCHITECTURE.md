...
│   ├── synth.py             # Waveforms + drones + pads (seeded jitter_cents for anti-beating) + granular
...
│       ├── record_piano.py  # mido + python-rtmidi MIDI capture
│       ├── render_midi.py   # FluidSynth subprocess wrapper (offline MIDI->WAV)
│       ├── process_piano.py # Audiomancer chain (bass_drone / mid_pad / sparse_notes presets)
│       └── README.md        # 3-command workflow documentation
├── tests/                   # 18 test files, 367 tests passing
